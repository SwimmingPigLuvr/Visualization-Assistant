<script lang="ts">
    ...
    $: isValid = newUsername?.length > 2 && newUsername.length < 16 && re.test(newUsername);
        ...
        <p>signed in as <span class="text-white">{username}</span></p>
            <form class="flex flex-col space-y-2 w-full" on:submit|preventDefault={confirmUsername}>
                    ...
                    {#if isTouched}
                        {#if !isValid}
                            <div in:slide out:slide class="flex flex-col">
                                <p class="text-red-400 text-xs">must be 3-16 characters long</p>
                        ...
                        {:else}
                            <p>status: </p>
                            {#if loading}
                                <img class="animate-spin h-6 w-6 rounded-full" src="/icons/gigaBubble.png" alt="">
                            {/if}
                            {#if isAvailable}
                ...
                {#if isValid && isAvailable}
                    <button class="bg-lime-600 text-slate-200 text-xs p-1 hover:bg-lime-400">confirm username <span class="text-white">{newUsername}</span></button>
                {/if}
            </form>
    </div>
