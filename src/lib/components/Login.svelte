    ...
    async function signInWithGoogle() {
        ...
        try {
            if (browser) {
                ...
                if (isMobile) {
                    console.log("attemting mobile sign in...");
...
<SignedOut>
    ...
    {#if $signInModalOpen}
        <button
            ...
            class="z-40 w-full h-screen bg-black bg-opacity-75 inset-0 fixed top-0 left-0 p-2"
        >
            <button
                on:click|stopPropagation
                class="z-50 flex items-center flex-col space-y-4 w-full max-w-[24rem] relative m-auto p-8 border-white border-[1px] bg-black"
            >
                <form
                ...
                >
                    <button
                        class="rounded-bl-3xl absolute top-0 right-0 rounded-none px-4 p-2 text-xs border-white border-[1px]"
                        on:click={() => signInModalOpen.set(false)}>X</button
                    >
                    <label for="email">email</label>
                    <input
                        class="text-black px-2 p-1"
                        type="email"
                        name="email"
                        id="email"
                        bind:value={email}
                    ...
                    <input
                        ...
                        bind:value={password}
                    />
                    <button class="border-white border-[1px] p-1" type="submit"
                        >submit</button
                    >
                </form>
                ...
                <!-- google -->
                <button
                    class="p-2 px-4 w-full bg-slate-400 rounded-none"
                    on:click={signInWithGoogle}>Google</button
                >
                <p>
                    {isSignUp
